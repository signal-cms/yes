...
class LaiThread:
    ...
    def __init__(self, log_path=None, list_url=None, room_url=None, error_log=None, run_log=None, database_dict=None, current_rank_url=None):
        ...
        default_database_dict = {'host': 'localhost', 'port': 3306, 'user': 'tester', 'password': 'Test_1357642', 'db': 'laifeng'}
    ...
    def update_living_list(self, living_list):
        ...
        # mark sql
        mark_content = '{0}_{1}'.format(living_num+1, datetime.now().strftime("%Y_%m_%d_%H_%M_%S"))
        ...
        # the last sql
        sql_list.append(r"INSERT INTO {0}(id, content) VALUES({1}, '{2}') ON DUPLICATE KEY UPDATE content='{2}'".format(self.living_db, i+2, self.join_num(living_list[(group_num-1)*10:])))
    ...
    def _get_num(self, html_file, allow_zero):
        ...
        if not star_corn:
            ...
            if not pat_star_corn.findall(html_file):
                print(star_corn)
                print(html_file)
        ...
        for room_info in room_list:
            ...
            try:
                sql_command = "INSERT INTO {6}(roomid,peo,xingbi,renqi,online,time) VALUES ('{0}','{1}','{2}','{3}','{4}','{5}')".format(room_info[0], room_info[1].replace("'", ''), data[0], data[1], data[2],
                                                                                                                                         datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.detail_db)
    ...
    def _write_offline_data(self, room_list, offline_info_dict):
        ...
        for roomid in room_list:
            js = offline_info_dict.get(roomid)
            ...
            if data:
                ...
                for info in data:
                    ...
                    sql_list.append(insert_sql.format(self.consumer_db, user_id, user_name, star_coin, roomid, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        self.execute_db_more(sql_list)

    def _offline_task(*room_list):
        offline_info_dict = {}
        ...
        offline_bank = []
        for i in range(10):
            offline_th = threading.Thread(target=self.request_get_more, args=(offline_task_list[i], offline_info_dict,))
    ...
    def run_spider(self):
        ...
        if living_mark:
            ...
            if (nowtime - mark_date).seconds < 1800:
                print(living_mark[0][1])
        ...
        if last_living_list:
            for roomid in last_living_list:
                if roomid not in living_list:
                   off_line_list.append(roomid) 
        if off_line_list:
            for roomid in off_line_list:
                room_list.append([roomid, '下播检查', 1])
